...
class DetailActivity : AppCompatActivity() {
    ...
    /** 下载视频（m3u8 用前台服务，直链用系统 DownloadManager） */
    private fun startDownload() {
        ...
        if (firstUrl.contains(".m3u8", ignoreCase = true)) {
            // m3u8 → 前台服务下载（后台不中断，通知栏实时进度）
            if (DownloadService.isDownloading(firstUrl)) {
            ...
            DownloadService.start(this, firstUrl, video.name)
            ...
            try {
                ...
                val request = android.app.DownloadManager.Request(android.net.Uri.parse(firstUrl))
                    .setTitle(video.name)
